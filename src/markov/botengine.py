"""
마르코프 체인 클래스가 정의된 botengine.py 파일입니다.
...
자세한 코드는 아래에 구현되어있습니다.
책에 있는 코드를 무작정 따라서 작성하는 것 보다 구체적인 설명을 따라 읽는 것이
여러분이 마르코프 체인기법을 이해하는데 더욱 도움 될 것 같아서 모든 코드에 주석을 첨부하였습니다.
코드를 작성하지 마시고, 주석을 쭉 따라 읽으시면서 이해가 안가는 부분이 생기신다면 질문 해주세요!
